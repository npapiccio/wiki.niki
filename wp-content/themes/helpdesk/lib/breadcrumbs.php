  ...
  if ( is_singular() ) {
    ...
    if ( 0 == $post->post_parent ) {
      ...
      if ( count( $terms ) > 0 ) {
        ...
        if ( is_taxonomy_hierarchical( $main_tax ) && $deepest_term->parent != 0 ) {
          foreach ( get_term_parents( $deepest_term ) as $parent_term ) {
    ...
    if ( is_post_type_archive() ) {
      ...
      if ( is_taxonomy_hierarchical( $term->taxonomy ) && $term->parent != 0 ) {
        foreach ( get_term_parents( $term ) as $parent_term ) {
          $links[] = array( 'term' => $parent_term );
        }
      }
      ...
      $bc      = __( 'You searched for', 'pressapps' );
      ...
      if ( 0 !== get_query_var( 'year' ) || ( 0 !== get_query_var( 'monthnum' ) || 0 !== get_query_var( 'day' ) ) ) {
        ...
        $crumb404 = __( 'Error 404: Page not found', 'pressapps' );
        $links[] = array( 'text' => $crumb404 );
      }
    }
  }

  $output = create_breadcrumbs_string( $links );
